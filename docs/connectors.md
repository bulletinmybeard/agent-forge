...
The flow is REST-driven (`web/server/connectors/api.py`). OAuth connectors (Google) use `auth/start` + `auth/callback`; token connectors (GitLab) use `auth/token`.
...
| POST   | `/api/connectors/auth/token`      | Create a token connection. Body: `{"connector_type": "gitlab", "url": "...", "token": "...", "read_write": false, "label": "..."}`. |
...
- [modes.md](modes.md): the `@conn` / `@google` / `@gitlab` connector modes this wires up.
