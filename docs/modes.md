...
| `@conn`, `@connector` | connector  | External service connectors (Gmail, Google Drive, GitLab). See [gcp-oauth-gmail.md](gcp-oauth-gmail.md). |
...
`@conn` reaches external accounts you have linked: Gmail, Google Drive, and GitLab, with multi-account support. Each connection gets a label that doubles as a `#hashtag` for targeting (`@conn #work-gitlab list my open MRs`); without a tag the mode routes by keyword. Connector tools run in-process (they are bound to each connection's credentials) rather than on the worker queue. Setup is in [gcp-oauth-gmail.md](gcp-oauth-gmail.md).
