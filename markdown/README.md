...
- `custom-agents/` — per-agent system prompts referenced by `custom_agents.yaml` / `custom_agents.example.yaml` (`system_prompt:`). One file per custom agent mode (`@docker`, `@debug`, `@trip`, ...). Private prompts (e.g. `@afemail`) go in `markdown/local/` (gitignored).
